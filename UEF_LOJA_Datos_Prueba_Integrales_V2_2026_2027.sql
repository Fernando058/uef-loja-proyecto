...
--   * 300 matrículas
--   * 756 evaluaciones ordinarias
--   * 18.900 calificaciones iniciales
...
-- ---------------------------------------------------------------------
-- 0. PROTECCIÓN
-- Evita ejecutar este conjunto sobre una base que ya tenga alumnado real.
-- Se permiten únicamente estudiantes TEST-*.
-- ---------------------------------------------------------------------
...
begin
  if exists (
    ...
    where national_id is null
       or national_id not like 'TEST-%'
  ) then
    raise exception
      'Se detectaron estudiantes que no son de prueba. Este script está protegido y no continuará.';
  end if;
end
$$;
