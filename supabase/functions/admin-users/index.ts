...
Deno.serve(async (request) => {
  ...
  if (!supabaseUrl || !anonKey || !serviceRoleKey) {
    return json({ error: 'La función no tiene las variables internas de Supabase.' }, 500)
  }
  ...
  try {
    ...
    if (callerError || caller?.role !== 'director' || !caller.active) {
      return json({ error: 'Solo el director puede administrar cuentas.' }, 403)
    }
    ...
    if (action === 'create') {
      ...
      const role = body.role === 'director' ? 'director' : 'teacher'
      const teacherId = body.teacher_id ? String(body.teacher_id) : null

      if (!email || !firstNames || !lastNames || password.length < 8) {
        return json({ error: 'Datos incompletos o contraseña menor a 8 caracteres.' }, 400)
      }
      if (role === 'teacher' && !teacherId) {
        return json({ error: 'Debe seleccionar el registro docente.' }, 400)
      }
      ...
      const { data: created, error: createError } = await adminClient.auth.admin.createUser({
        ...
        password,
        email_confirm: true,
        user_metadata: { first_names: firstNames, last_names: lastNames },
      })
      ...
      try {
        ...
        if (role === 'teacher' && teacherId) {
          const { data: linked, error: teacherError } = await adminClient
            .from('teachers')
            .update({ user_id: created.user.id, first_names: firstNames, last_names: lastNames })
            .eq('id', teacherId)
            .is('user_id', null)
            .select('id')
            .maybeSingle()
    ...
    if (action === 'set_active') {
      ...
      if (userId === authData.user.id && !active) {
        return json({ error: 'No puede deshabilitar su propia cuenta.' }, 400)
      }
