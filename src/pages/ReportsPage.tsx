...
export function ReportsPage() {
  ...
  return <>
    ...
    {tab === 'individual' ? <div className="report-stack"><div className="report-title-block"><p>Resumen anual por estudiante</p><h2>{selectedStudent ? fullName(selectedStudent.first_names, selectedStudent.last_names) : 'Seleccione un estudiante'}</h2></div><section className="panel"><div className="table-wrap"><table><thead><tr><th>Asignatura</th><th>T1</th><th>T2</th><th>T3</th><th>Promedio anual</th><th>Cualitativa</th></tr></thead><tbody>{annualResults.map((row) => <tr key={row.subject_id}><td><strong>{subjectName(row.subject_id)}</strong></td><td>{formatScore(row.term_1)}</td><td>{formatScore(row.term_2)}</td><td>{formatScore(row.term_3)}</td><td>{formatScore(row.annual_score)}</td><td><span className="badge badge-primary">{row.qualitative ?? '—'}</span></td></tr>)}{!annualResults.length && <tr><td colSpan={6} className="empty-cell">Todavía no existen resultados anuales completos.</td></tr>}</tbody></table></div></section><section className="panel chart-panel"><div className="panel-heading"><div><h2>Rendimiento anual por asignatura</h2></div></div><div className="chart-box"><ResponsiveContainer width="100%" height="100%"><BarChart data={individualData}><CartesianGrid stroke="#d9e2ec" strokeDasharray="3 3" /><XAxis dataKey="subject" hide /><YAxis domain={[0,10]} stroke="#64748b" /><Tooltip /><Bar dataKey="score" fill="#2f6f9f" radius={[6,6,0,0]} /></BarChart></ResponsiveContainer></div></section></div> : <div className="report-stack"><section className="charts-grid"><article className="panel chart-panel"><div className="panel-heading"><div><h2>Promedio por asignatura</h2></div></div><div className="chart-box"><ResponsiveContainer width="100%" height="100%"><BarChart data={courseSubjectGroups}><CartesianGrid stroke="#d9e2ec" strokeDasharray="3 3" /><XAxis dataKey="subject" hide /><YAxis domain={[0,10]} stroke="#64748b" /><Tooltip /><Bar dataKey="average" fill="#2f6f9f" radius={[6,6,0,0]} /></BarChart></ResponsiveContainer></div></article><article className="panel chart-panel"><div className="panel-heading"><div><h2>Distribución de desempeño</h2></div></div><div className="chart-box">{allCourseScores.length ? <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={pieData} dataKey="value" nameKey="name" outerRadius={100} label>{pieData.map((_, index) => <Cell key={index} fill={PIE_COLORS[index]} />)}</Pie><Tooltip /></PieChart></ResponsiveContainer> : <div className="chart-empty">Sin resultados calculados para este trimestre.</div>}</div></article></section><section className="panel"><div className="table-wrap"><table><thead><tr><th>Estudiante</th>{subjects.map((subject) => <th key={subject.id}>{subject.abbreviation}</th>)}</tr></thead><tbody>{tableRows.map(({ enrollment, scores }) => <tr key={enrollment.id}><td><strong>{fullName(enrollment.student?.first_names, enrollment.student?.last_names)}</strong></td>{subjects.map((subject) => <td key={subject.id}>{formatScore(scores[subject.id])}</td>)}</tr>)}{!tableRows.length && <tr><td colSpan={subjects.length + 1} className="empty-cell">No existen resultados en este curso.</td></tr>}</tbody></table></div></section></div>}
  </>
}
