...
export function AttendancePage() {
  ...
  const loadAttendance = useCallback(async () => {
    ...
    if (error) throw error
    const rows = (data ?? []) as AttendanceSummary[]

    setDrafts(Object.fromEntries(
      enrollmentRows.map((enrollment) => {
        const row = rows.find((item) => item.enrollment_id === enrollment.id)
        return [
          enrollment.id,
          row
            ? {
    ...
    try {
      const payload = enrollments.map((enrollment) => {
        ...
        return {
          ...
          term_id: termId,
          attended_days: nonNegativeInteger(draft.attended_days),
          justified_absences: nonNegativeInteger(draft.justified_absences),
          unjustified_absences: nonNegativeInteger(draft.unjustified_absences),
  ...
  return (
    <>
      <PageHeader
        ...
        description="Complete al final de cada trimestre los días asistidos y las faltas. No se registra asistencia diaria."
        actions={canEdit ? (
          <button
      ...
      {!canEdit && (
        <div className="alert alert-warning">
          La cuenta docente puede consultar; la edición de los totales corresponde al director.
        </div>
      )}
      ...
      <section className="panel attendance-panel">
        <div className="panel-heading">
          <div>
            ...
            <p>El director puede corregir estos valores posteriormente.</p>
          </div>
        </div>
          ...
          <table className="attendance-table">
            ...
            <tbody>
              {enrollments.map((enrollment) => {
                const draft = drafts[enrollment.id] ?? emptyDraft()
                const total =
                  nonNegativeInteger(draft.attended_days)
                  + nonNegativeInteger(draft.justified_absences)
                  + nonNegativeInteger(draft.unjustified_absences)
                  ...
                  <tr key={enrollment.id}>
                    ...
                    <td>
                      <input
                        ...
                        min="0"
                        disabled={!canEdit}
                        value={draft.attended_days}
                        onChange={(event) =>
                          updateDraft(enrollment.id, { attended_days: event.target.value })
                        }
                      />
                    </td>
                    <td>
                      <input
                        ...
                        min="0"
                        disabled={!canEdit}
                        value={draft.justified_absences}
                        onChange={(event) =>
                          updateDraft(enrollment.id, { justified_absences: event.target.value })
                        }
                      />
                    </td>
                    <td>
                      <input
                        ...
                        min="0"
                        disabled={!canEdit}
                        value={draft.unjustified_absences}
                        onChange={(event) =>
                          updateDraft(enrollment.id, { unjustified_absences: event.target.value })
                        }
                      />
                    </td>
                    ...
                    <td>
                      <input
                        ...
                        value={draft.notes}
                        onChange={(event) =>
                          updateDraft(enrollment.id, { notes: event.target.value })
                        }
              ...
              {!enrollments.length && (
                <tr>
                  <td colSpan={6}>No existen estudiantes matriculados.</td>
                </tr>
              )}
            </tbody>
          </table>
